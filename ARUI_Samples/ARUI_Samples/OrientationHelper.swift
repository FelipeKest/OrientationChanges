        ...
        motionManager.startAccelerometerUpdates(to: queue) { (data, error) in
            if let accelerometerData = data {
                ...
                if newDeviceOrientation != self.currentDeviceOrientation {
                    ...
                    if let orientationHandler = self.deviceOrientationAction {
                        DispatchQueue.main.async {
                            guard let orientationHandler = orientationHandler else {return}
                            orientationHandler(self.currentDeviceOrientation)
                        }
                    }
                }
            }
